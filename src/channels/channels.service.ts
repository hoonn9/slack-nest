...
export class ChannelsService {
  constructor(
    ...
    private usersRepository: Repository<Users>, // private readonly
  ...
  async createWorkspaceChannelMembers(url, name, email) {
    const channel = await this.channelsRepository
      ...
      .getOne();
    if (!channel) {
      return new NotFoundException('채널이 존재하지 않습니다.'); // TODO: 이 때 어떻게 에러 발생?
    }
    const user = await this.usersRepository
      ...
      .getOne();
    if (!user) {
      return new NotFoundException('사용자가 존재하지 않습니다.');
    }
    const channelMember = new ChannelMembers();
